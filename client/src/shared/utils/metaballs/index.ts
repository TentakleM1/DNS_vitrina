export const metaballs = (canvas: HTMLCanvasElement) => {
    const gl:  WebGLRenderingContext | null = canvas.getContext('webgl');
    const width = canvas.width
    const height = canvas.height
  
  const numMetaballs: number = 10;
  const metaballs = [];
...
void main(){
  ...
  if (sum > 1.0) {
    gl_FragColor = vec4(1.0, 0.5, 0.0, 0.9); // Оранжевый цвет
  } else {
    gl_FragColor = vec4(0.0, 0.65, 1.0, 1.0); // Темно-синий фон
  }
}
